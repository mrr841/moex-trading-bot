...
class MarketRegimeDetector:
    ...
    def _determine_secondary_regimes(self,
        ...
        # Режим ликвидности
        if liquidity > 1.5:
            regimes.append(MarketRegime.LIQUID)
        elif liquidity < 0.5:
            regimes.append(MarketRegime.ILLIQUID)
            
        # Дополнительные проверки волатильности
        if volatility > self.volatility_thresholds['high'] * 1.5:
            regimes.append(MarketRegime.HIGH_VOLATILITY)
        elif volatility < self.volatility_thresholds['low'] * 0.5:
